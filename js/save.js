const SaveManager = (() => {
    ...
    function xorEncrypt(str, key) {
        ...
        const result = new Uint8Array(utf8Bytes.length);
        for (let i = 0; i < utf8Bytes.length; i++) {
            result[i] = utf8Bytes[i] ^ keyBytes[i % keyBytes.length];
        }
        let binary = '';
    ...
    function autoSave(data) {
        ...
        try {
            ...
            if (typeof AntiCheat !== 'undefined') {
                const signed = AntiCheat.signPlayerDataSync(saveData);
                saveData.__sig = signed.sig;
                saveData.__ts = signed.ts;
            }
    ...
    function loadAutoSave() {
        try {
            ...
            if (typeof AntiCheat !== 'undefined' && data.__sig !== undefined) {
                const clone = { ...data };
                delete clone.__sig;
                delete clone.__ts;
                const valid = AntiCheat.verifyPlayerDataSync(clone, data.__sig);
                if (!valid) {
                    if (typeof AntiCheat.flagSuspicious === 'function') {
                        AntiCheat.flagSuspicious('autosave_signature_invalid');
                    }
                    return { success: false, message: 'Save data signature invalid' };
                }
            }
    ...
    function loadFromSlot(slotIndex) {
        try {
            ...
            if (typeof AntiCheat !== 'undefined' && data.__sig !== undefined) {
                ...
                if (!valid) {
                    if (typeof AntiCheat.flagSuspicious === 'function') {
                        AntiCheat.flagSuspicious('slot_signature_invalid_' + slotIndex);
                    }
                    return { success: false, message: 'Slot ' + slotIndex + ' signature invalid' };
                }
            }
            return { success: true, data: data };
        } catch (e) {
    ...
    // ============================================================
    // JSON 存档验证码机制（具体算法不公开，避免针对性绕过）
    // ============================================================
    function _decodeTargetDate() {
        const parts = ['MjAyNg==', 'MDctMjQ=', 'MjE6Mzk6MDA=', 'KzA4OjAw'];
        return atob(parts[0]) + '-' + atob(parts[1]) + 'T' + atob(parts[2]) + atob(parts[3]);
    }
    const VERIFICATION_TARGET = new Date(_decodeTargetDate());

    function computeDigitSums(dataString) {
        let evenSum = 0;
        let oddSum = 0;
        for (let i = 0; i < dataString.length; i++) {
            const digit = parseInt(dataString[i], 10);
    ...
    function computeVerificationCode(dataString, timestamp) {
        const saveTime = timestamp ? new Date(timestamp) : new Date();
        const secondsDiff = Math.floor(Math.abs((VERIFICATION_TARGET - saveTime) / 1000));
        const { evenSum, oddSum } = computeDigitSums(dataString);
        if (evenSum === 0 || oddSum === 0) return '0';
        return (secondsDiff * evenSum * oddSum).toString();
    }

    function padVerificationCode(code) {
        if (code.length >= 64) return code;
        const paddingCount = 64 - code.length;
        let padding = '';
    ...
    function attachVerificationCode(data) {
        const core = {
        ...
        const rawCode = computeVerificationCode(coreJson, core.timestamp);
        const paddedCode = padVerificationCode(rawCode);
        const paddingCount = paddedCode.length - rawCode.length;
        return {
            ...core,
            version: '1.0.' + paddingCount,
            verificationCode: paddedCode
        };
    }

    function verifySaveCode(data) {
        if (!data || !data.verificationCode) {
            return { valid: false, message: '缺少验证码' };
        }
        const versionParts = (data.version || '1.0.0').split('.');
        const paddingCount = parseInt(versionParts[versionParts.length - 1], 10) || 0;
        const codeWithPadding = data.verificationCode.toString();
        const actualCode = codeWithPadding.slice(0, codeWithPadding.length - paddingCount);
        ...
        const coreJson = JSON.stringify(core);
        const expected = computeVerificationCode(coreJson, core.timestamp);

        if (actualCode !== expected) {
            return { valid: false, message: '验证码校验失败，存档可能已被篡改' };
        }
        return { valid: true };
    }
    ...
    function exportSave() {
        ...
        const verifiedData = attachVerificationCode(result.data);
        return JSON.stringify(verifiedData, null, 2);
    }
    ...
    function importSave(jsonString) {
        ...
        try {
            ...
            const codeCheck = verifySaveCode(data);
            if (!codeCheck.valid) {
                return {
                    success: false,
                    message: '验证码校验失败: ' + codeCheck.message
                };
            }
